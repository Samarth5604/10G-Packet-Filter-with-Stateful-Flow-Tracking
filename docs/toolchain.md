...
Then build once before opening an editor:

```
dune build
```

`ocamllsp` resolves modules from `_build`. With no build context it reports
every cross-module reference as unbound -- "No config found for file ...,
try calling 'dune build'" followed by a cascade of `Unbound module`,
`Unbound constructor` and `Unbound value`. These are not code errors. Build
first.

`packet_filter.opam` is generated by dune from the `package` stanza in
`dune-project`, but is committed: `opam install . --deps-only` reads it before
dune has run.
